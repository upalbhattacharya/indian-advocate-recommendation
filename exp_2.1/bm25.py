...
# Modify: 2022-08-31 13:02:46.247757924 +0530
...
def main():
    ...
    cases = list(set(chain.from_iterable([cases["train"] for cases in
                                         high_count_advs.values()])))

    logging.info(f"Found {len(cases)} database cases")
    train_texts = get_text(cases, args.file_path)
    ...
    logging.info("Getting document frequencies of queries")
    test_doc_freqs = get_doc_freqs(
        test_texts, dictionary, drop_tokens)

    logging.info("Computing BM25 scores")
    scores = {
    ...
    write_to_dir(unique_doc_lens, [args.output_path, "model"],
                 "unique_doc_lens")
    write_to_dir(test_doc_freqs, [args.output_path, "model"],
                 "test_doc_freqs")

...
def get_text(doc_id_list, data_path):
...
def main():
    ...
    cases = list(set(chain.from_iterable([cases["train"] for cases in
                                         split_info.values()])))

    logging.info(f"Found {len(cases)} database cases")
    train_texts = get_text(cases, args.data_path)
    test_doc_ids = set(chain.from_iterable([cases["test"] for cases in
                                            split_info.values()]))
    logging.info(f"Found {len(test_doc_ids)} query cases")
    ...
    bm25 = BM25Okapi(list(train_texts.values()))
    scores = {}
    ...
    for idx, text in test_texts.items():
        ...
        scores[idx] = {
                k: v for k, v in sorted(zip(train_texts.keys(), scores[idx]),
                                        key=lambda x: x[1],
                                        reverse=True)}

    vectorizer = TfidfVectorizer()
    logging.info("Generating tf-idf vectors of advocate representations")
    train_vectors = vectorizer.fit_transform([" ".join(text)
    ...
    train_embed_path = os.path.join(args.embedding_path, "train_rep")
    if not os.path.exists(train_embed_path):
        os.makedirs(train_embed_path)

    for idx, vector in zip(train_texts.keys(), train_vectors):
        logging.info(f"Saving tf-idf rep of {idx}")
        with open(os.path.join(train_embed_path, f"{idx}.npy"), 'wb') as f:

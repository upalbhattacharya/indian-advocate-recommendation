...
# -*- encoding: utf-8 -*-
# Birth: 2022-09-12 19:44:34.818051050 +0530
# Modify: 2022-09-12 20:39:28.810262081 +0530
...
def f1(outputs_batch: MutableSequence,
    ...
    scores = {
        ...
        'f1': per_class_f1,
        'sup': per_class_sup,
        'macro_prec': (float(macro_prec) if macro_prec is not None else
                       macro_prec),
        'macro_rec': (float(macro_rec) if macro_rec is not None else
                      macro_rec),
        'macro_f1': (float(macro_f1) if macro_f1 is not None else
                     macro_f1),
        'macro_sup': (float(macro_sup) if macro_sup is not None else
                      macro_sup),
        'micro_prec': (float(micro_prec) if micro_prec is not None else
                       micro_prec),
        'micro_rec': (float(micro_rec) if micro_rec is not None else
                      micro_rec),
        'micro_f1': (float(micro_f1) if micro_f1 is not None else
                     micro_f1),
        'micro_sup': (float(micro_sup) if micro_sup is not None else
                      micro_sup)
        }

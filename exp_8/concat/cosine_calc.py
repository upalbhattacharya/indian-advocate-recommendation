...
# -*- encoding: utf-8 -*-
# Birth: 2022-09-13 09:15:38.512633676 +0530
# Modify: 2022-09-13 09:22:27.173146030 +0530
...
def cosine(a, b):
    return float(torch.dot(
        a, b).item()/(torch.linalg.norm(a).item()*torch.linalg.norm(b).item()))
...
def main():
    ...
    for fl in os.listdir(args.database_embed_path):
        ...
        with open(os.path.join(args.database_embed_path, fl), 'rb') as f:
            db_embeds[flname] = torch.load(f)
            print(db_embeds[flname].shape)

    for fl in os.listdir(args.query_embed_path):
        flname = os.path.splitext(fl)[0]
        with open(os.path.join(args.query_embed_path, fl), 'rb') as f:
            q_embeds[flname] = torch.load(f)
            print(q_embeds[flname].shape)

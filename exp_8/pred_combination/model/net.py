...
class PredVectorCombination(nn.Module):
    def __init__(self, labels, input_dims, proj_dim=300, names=None,
                 device='cpu'):
        super(PredVectorCombination, self).__init__()
        ...
        self.self_attn = ScaledDotProductAttention(features=self.proj_dim,
                                                   device=self.device)
        self.linear_combination = nn.Parameter(torch.Tensor(1, self.proj_dim))
    ...
    def forward(self, *x):
        ...
        relu = nn.ReLU()
        projs = relu(projs)
        logging.info(f"Shape of projected input: {projs.shape}")
        #  projs = self.self_attn(projs, projs, projs)
        softmax = nn.Softmax()
        self.linear_combination = softmax(self.linear_combination)
        projs = torch.einsum("ij, bkj -> bij", self.linear_combination, projs)
        #  projs = self.mean_pool(projs)
        logging.info(
                f"Shape of final projection: {projs.shape}")
        ...
        logging.info(f"Shape of final prediction: {preds.shape}")

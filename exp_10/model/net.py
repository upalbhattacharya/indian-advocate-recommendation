...
class SimpleMultiTaskMultiLabelPrediction(nn.Module):
    ...
    def __init__(
        ...
        self._labels = {
            k: [re.sub(r"[^A-Za-z]", "", label) for label in v]
        ...
        if truncation_side not in ["right", "left"]:
            raise ValueError(
                (
                    "Truncation must be 'right' or 'left'. "
        ...
        if mode not in ["train", "generate"]:
            raise ValueError(
                ("Mode must be 'train' or 'generate'. " f"Found {mode}")
            )
        self._mode = mode
        ...
        self._tokenizer = AutoTokenizer.from_pretrained(
            self._model_name, truncation_side=self._truncation_side
        )

        # Model layers
        self.embedding = AutoModel.from_pretrained(self._model_name)

        self.projection_adv = nn.Sequential(
            OrderedDict(
                [
                    (
                        "linear1",
                        nn.Linear(
                            in_features=768, out_features=1024, bias=True
                        ),
                    ),
                    ("relu1", nn.ReLU()),
                    (
        ...
        self.projection_area = nn.Sequential(
            OrderedDict(
                [
                    (
                        "linear1",
        ...
        self.prediction_adv = nn.ModuleDict(
            {
                k: nn.Linear(in_features=128, out_features=1, bias=True)

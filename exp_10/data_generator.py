...
class MultiTaskDataset(Dataset):
    ...
    def __init__(
        ...
        if not isinstance(targets_paths, dict):
            raise TypeError(
                (
                    "'targets_paths' must be of type 'dict'. "
        ...
        if not all([k in ["adv", "area"] for k in targets_paths.keys()]):
            raise ValueError(
                (
                    "Only targets_paths keys 'adv' and 'area' "
        ...
        if not isinstance(unique_labels, dict):
            raise TypeError(
                (
                    "'unique_labels' must be of type 'dict'. "
        ...
        if not all([k in ["adv", "area"] for k in unique_labels.keys()]):
            raise ValueError(
                (
                    "Only unique_labels keys 'adv' and 'area' "
    ...
    def __getitem__(
        self, idx: int
    ) -> Tuple[Union[str, torch.Tensor], torch.Tensor, torch.Tensor]:
    ...
    @property
    def idx(self) -> Dict[str, str]:
        """Gettr for unique_labels"""
        return self._idx

    @property
    def targets_dict(self) -> Dict[str, str]:
        """Gettr for unique_labels"""
    ...
    def _get_unique_labels(
        self, unique_labels: Dict[str, Union[str, None]]
    ) -> Dict[str, List[str]]:
        u_labels = {}
                        ...
                        chain.from_iterable(
                            list(
                                filter(
                                    None,
                                    map(
                                        lambda x: self._targets_dict[x][key],

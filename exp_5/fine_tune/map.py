...
# Birth: 2022-07-21 16:48:02.882297203 +0530
# Modify: 2022-07-26 10:53:24.317977099 +0530
...
def one_query_ap(precisions, y_true, relevance):
    ...
    relevance values."""
    dot = np.dot(precisions, relevance[:len(precisions)])
    sum = np.sum(y_true)
    if sum != 0:
        return dot * 1./(np.sum(y_true))
    else:
        return 0.0
